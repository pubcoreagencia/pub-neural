...
-- Migration File: 0005_knowledge_governance_gates.sql
-- Classification: KNOWLEDGE GOVERNANCE / VALIDATION & ADOPTION GATES (V0.1)
-- Target: PostgreSQL 16+ / 17+
-- ============================================================================

\set ON_ERROR_STOP on

-- Update reduce_event to acknowledge sovereign milestone/governance events as SUPPORTED
CREATE OR REPLACE FUNCTION pub_neural.reduce_event(
    p_event pub_neural.neural_events
) RETURNS VARCHAR(32)
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = pg_catalog, pub_neural, public, extensions
AS $$
DECLARE
    ...
    v_target_node pub_neural.neural_nodes%ROWTYPE;
BEGIN
    ...
    -- - Target node current promotion_state must be 'CANDIDATE' (or 'VALIDATED' for idempotent re-validation)
    -- - Target node conflict_state MUST NOT be in ('CONTRADICTORY', 'BLOCKED', 'SUPERSEDED', 'DEPRECATED', 'REJECTED')
    -- - Actor MUST NOT be AGENT (requires sovereign/authorized role: CEO, ADMIN, or AUDITOR)
    -- - Evidence requirement: requires verifiable evidence_id, source_id, or valid evidence link
    ELSIF p_event.event_type = 'KNOWLEDGE_VALIDATED' THEN
        IF p_event.payload->>'target_id' IS NULL THEN
            RETURN 'MALFORMED';
        END IF;

        -- Check actor authority (FAIL CLOSED for AGENT)
        IF p_event.actor_role = 'AGENT' THEN
            RETURN 'REJECTED';
        END IF;

        SELECT * INTO v_target_node
        FROM pub_neural.neural_nodes
        WHERE id = p_event.payload->>'target_id' AND is_active = TRUE;

        IF NOT FOUND THEN
            RETURN 'REJECTED';
        END IF;

        -- Contradiction Gate: cannot promote nodes under conflict or rejection
        IF v_target_node.conflict_state IN ('CONTRADICTORY', 'BLOCKED', 'SUPERSEDED', 'DEPRECATED', 'REJECTED') THEN
            RETURN 'REJECTED';
        END IF;

        -- Transition Check: Must be in CANDIDATE state (or already VALIDATED for idempotency)
        IF v_target_node.promotion_state NOT IN ('CANDIDATE', 'VALIDATED') THEN
            RETURN 'REJECTED';
        END IF;

        -- Evidence Gate: must provide evidence reference or link
        IF (p_event.payload->>'evidence_id' IS NULL)
           AND (p_event.payload->>'source_id' IS NULL)
           AND (p_event.payload->>'evidence_summary' IS NULL)
           AND NOT EXISTS (SELECT 1 FROM pub_neural.neural_evidence WHERE node_id = v_target_node.id)
           AND NOT EXISTS (SELECT 1 FROM pub_neural.neural_edges WHERE source_id = v_target_node.id AND relation_type = 'DERIVED_FROM') THEN
            RETURN 'REJECTED';
        END IF;

        UPDATE pub_neural.neural_nodes
        SET promotion_state = 'VALIDATED',
            promotion_reason = 'Validated by ' || p_event.actor_id || ' (' || p_event.actor_role || '): ' || COALESCE(p_event.payload->>'validation_reason', p_event.payload->>'verification_method', 'evidence verified'),
            last_transition_event_id = p_event.id,
            updated_at = p_event.recorded_at
    ...
    -- 4b. KNOWLEDGE_ADOPTED -> promotion_state = 'ADOPTED'
    -- GOVERNANCE GATE:
    -- - Target node must exist and be active
    -- - Target node current promotion_state must be 'VALIDATED' (or 'ADOPTED' for idempotent re-adoption)
    -- - Target node conflict_state MUST NOT be in ('CONTRADICTORY', 'BLOCKED', 'SUPERSEDED', 'DEPRECATED', 'REJECTED')
    -- - Actor MUST be sovereign (CEO or ADMIN)
    ELSIF p_event.event_type = 'KNOWLEDGE_ADOPTED' THEN
        IF p_event.payload->>'target_id' IS NULL THEN
            RETURN 'MALFORMED';
        END IF;

        -- Check actor authority (requires CEO or ADMIN)
        IF p_event.actor_role NOT IN ('CEO', 'ADMIN') THEN
            RETURN 'REJECTED';
        END IF;
        ...
        IF v_target_node.promotion_state NOT IN ('VALIDATED', 'ADOPTED') THEN
            RETURN 'REJECTED';
        END IF;

        UPDATE pub_neural.neural_nodes
        SET promotion_state = 'ADOPTED',
            promotion_reason = 'Adopted by ' || p_event.actor_id || ' (' || p_event.actor_role || '): ' || COALESCE(p_event.payload->>'adoption_reason', 'operational pattern adopted'),
            last_transition_event_id = p_event.id,
            updated_at = p_event.recorded_at
        WHERE id = p_event.payload->>'target_id';
        ...
        IF (p_event.payload ? 'candidateFindings' AND jsonb_typeof(p_event.payload->'candidateFindings') = 'array')
           OR (p_event.payload ? 'candidate_findings' AND jsonb_typeof(p_event.payload->'candidate_findings') = 'array') THEN
            ...
            LOOP
                ...
                ELSIF v_finding_type = 'DECISION' THEN
                    v_finding_class := 'DECISION'::pub_neural.neural_entity_type;
                ELSIF v_finding_type = 'RULE' THEN
                    v_finding_class := 'RULE'::pub_neural.neural_entity_type;
                ELSE
    ...
    -- Passthrough supported canonical events (manifest/bootstrap/governance/milestones)
    ELSIF p_event.event_type IN (
        ...
        'ACTOR_REGISTERED',
        'ACTOR_REVOKED',
        'AUTONOMOUS_AGENT_VALIDATED',
        'OPERATING_MODE_ADOPTED'
    ) THEN
        RETURN 'SUPPORTED';
